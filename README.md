...
The source to the page, generated by DecafJS follows.  Note that all the HTML generated by the page (view) template and partials (header/footer) are in the source as expected.
...
<body>
<h1>Home Page</h1>
<p>Today is September 13, 2014</p>
<h1>Footer goes here.</h1>
</body>
</html>
```
...
For this part of the tutorial, you'll need to have mongodb installed and running.  You can install it on a mac using homebrew, or on ubuntu with apt-get.  I assume you know how to get it installed.

To embellish the client side, I'm going to add Twitter's Bootstrap HTML/CSS/JavaScript framework.  This is simple to do - just add a dependency for bootstrap to our bower.json, and we'll add a dependency for jQuery as well:

    "jquery" : "2.x",
    "bootstrap" : "3.x"

After adding those lines and doing a bower update, both jQuery and Bootstrap are added to our bower_components directory.

So far, we've created a Home controller that renders HTML dynamically using a Hogan template.  For this phase of the tutorial, we're going to need to serve static content, like jquery.min.js, and bootstrap.min.css.  This is trivial using JOLT's StaticServer class.  
